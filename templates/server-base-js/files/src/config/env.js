...
function withoutCredentials(uri) {
...
function loadConfig() {
  ...
  const readUrl = (name, fallback, purpose) => {
    const value = raw(name);
    if (value === undefined) return fallback;
...
export const mongodbUriForDisplay = withoutCredentials(config.mongodbUri);

/**
 * The resolved configuration, safe to log.
 *
 * Secrets are absent by construction rather than filtered out afterwards: a key added to the
 * config is not printed until somebody adds it here deliberately.
 */
