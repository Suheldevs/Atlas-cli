...
A Vite + React single-page app, wired to the API in `../server`. It is a real client, not a shell:
...
The access token is kept **in a module variable** in `src/lib/api.js` and nowhere else. Not
...
`src/lib/api.js` is the only file that knows this. A response interceptor unwraps `data`, so callers
use `http.get('/auth/me')` and receive the payload directly. Failures are normalised into an
...
```
src/
  lib/api.js              axios instance, in-memory token, envelope unwrap, 401 -> refresh -> retry
  context/AuthContext.jsx user + loading state, login / signup / logout, session restore on mount
